...
Result find_largest_consecutive_product(char[], int);

Result find_largest_consecutive_product(char num[], int length) {
    // TODO: Why can't I dynamically determine the length?
    size_t num_len = 1000; // sizeof(num) / sizeof(num[0]);
    int pos = length - 1;
    int product = 1, max_product = 1, offset = 0;
    int i = 0;

    //printf("num_len: %d, pos: %d\n", num_len, pos);
    ...
    Result result = { max_product, offset };
...
int main(void) {
    ...
    Result result = find_largest_consecutive_product(big_num, 5);

    printf(
        "Largest product: %d\nOffset: %d\nNumbers: %d %d %d %d %d\n",
        result.product,
        result.offset,
        big_num[result.offset],
        big_num[result.offset + 1],
        big_num[result.offset + 2],
        big_num[result.offset + 3],
        big_num[result.offset + 4]
    );
